...
/* the earth */
var earth_sphere = new ol.Sphere(6378137);
...
redraw = function(e) {
  ...
  circle = function(p, km, l) {
    // project the circle w.r.t web-mercator
    var newcircle = ol.geom.Polygon.circular(earth_sphere, p, km*1000, 32).transform('EPSG:4326', 'EPSG:3857'); // 32 vertices
    vectorLayer[l].getSource().addFeature(new ol.Feature(newcircle));
  };
  ...
  for(layer = 0; layer < layers.length; layer++) {
    ...
    //TODO context.fillStyle = layers[layer].color;

    /* accessible without shortcut */
    circle(earthmouse, timeavailable * speed, layer);
    ...
    for(i=0; i<nbcities; i++){
      ...
      if(remaining_distance > 0) {
        circle(pos[i], remaining_distance, layer);
      }
    }
  }
  ...
  var map = new ol.Map({
   //layer for the background map
    layers: [
      ...
      }), locationsLayer].concat(vectorLayer)
    ,
    renderer: 'canvas',
    target: 'map3857',
    // initial view
